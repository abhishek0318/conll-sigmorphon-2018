...
class GenerationProbabilty(nn.Module):
    def __init__(self, embedding_size, hidden_size):
        ...
        self.W_h_star = nn.Linear(2*hidden_size, 1, bias=False)
    ...
    def forward(self, h_star, s, x):
        ...
        Args:
            h_star: combined context vector over lemma and tag, of shape (bsz, 2*hidden_size)
            s: decoder hidden state, of shape (1, bsz, hidden_size)
...
class Encoder(nn.Module):
    def __init__(self, vocab, vocab_size, embedding_size, hidden_size):
    ...
    def forward(self, indices):
        ...
        Returns:
            ...
            h_n: final hidden state, of shape (1, bsz, hidden_size)
        """
...
class Decoder(nn.Module):
    def __init__(self, vocab, embedding_size, hidden_size, max_decode_len=25, epsilon=1e-6):
        ...
        self.generation_probability = GenerationProbabilty(embedding_size, hidden_size)
        self.generator = nn.Linear(3*hidden_size, vocab.char_vocab_size)

    def forward(self, lemma_input_indices, h_l, h_tg, mask_l, mask_tg, s_0, inputs=None, a_ls_true=None, p_gens_true=None):
        """
        ...
        s = s_0 # (1, bsz, hidden_size)
        c = torch.zeros(1, bsz, self.hidden_size, device=device)  # (1, bsz, hidden_size)
...
class Model(nn.Module):
    def __init__(self, vocab, embedding_size, hidden_size):
        ...
        self.vocab = vocab
        self.lemma_encoder = Encoder(vocab, vocab.char_vocab_size, embedding_size, hidden_size)
        self.tag_encoder = Encoder(vocab, vocab.tag_vocab_size, embedding_size, hidden_size)
        self.bridge = nn.Linear(2*hidden_size, hidden_size)
        self.decoder = Decoder(vocab, embedding_size, hidden_size)
    ...
    def forward(self, lemma_indices, tag_indices, inflected_form_indices=None, a_ls_true=None, p_gens_true=None):
        ...
        h_l, mask_l, h_l_n = self.lemma_encoder(lemma_indices)

        # (bsz, max_tag_len, 2*hidden_size), (bsz, max_tag_len), (1, bsz, hidden_size)
        h_tg, mask_tg, h_tg_n = self.tag_encoder(tag_indices)

        # (1, bsz, hidden_size) & (1, bsz, hidden_size) -> (1, bsz, hidden_size)
        s_0 = self.bridge(torch.cat([h_l_n, h_tg_n], dim=2))
        ...
        p_ws, a_ls, p_gens = self.decoder(lemma_indices_padded, h_l, h_tg, mask_l, mask_tg, s_0, decoder_input, a_ls_true, p_gens_true)
