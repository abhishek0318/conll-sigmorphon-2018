...
class Decoder(nn.Module):
    def __init__(self, vocab, embedding_size, hidden_size, use_ptr_gen, max_decode_len=25, epsilon=1e-6, dropout_p=0):
        super(Decoder, self).__init__()
        self.embedder = nn.Embedding(vocab.char_vocab_size+vocab.tag_vocab_size, embedding_size, padding_idx=vocab.padding_idx)
        self.vocab = vocab
    ...
    def forward(self, input_indices, h, mask, decoder_initial, inputs=None, a_ls_true=None, p_gens_true=None):
        """

        Args:
            input_indices: (bsz, max_lemma+tag_len) for p_gen
            h: (bsz, max_lemma_len, 2*hidden_size)
            mask: (bsz, max_lemma+tag_len)
        ...
        Returns:
            ...
            a_s: attention over input, of shape (bsz, max_decode_len, max_lemma+tag_len)
            p_gens: p_gens, of shape (bsz, max_decode_len)
        """

        bsz = h.shape[0]
        max_lemma_tag_len = h.shape[1]
        ...
        if inputs is not None:
            ...
            x = self.embedder(x) # (bsz, embedding_size)
        ...
        a_s = torch.zeros(bsz, self.max_decode_len, max_lemma_tag_len, device=device)  # (bsz, max_decode_len, max_lemma_tag_len)
        ...
        for t in range(max_t):
            ...
            x = self.embedder(p_w.argmax(dim=1))  # (bsz, embedding_size)
            p_ws[:, t, :] = p_w
            a_s[:, t, :] = a
            if self.use_ptr_gen:
                p_gens[:, t] = p_gen

        return torch.log(p_ws), a_s, p_gens


class Model(nn.Module):
    def __init__(self, vocab, embedding_size, hidden_size, use_ptr_gen, dropout_p=0):
        super(Model, self).__init__()
        self.vocab = vocab
        self.encoder = Encoder(vocab, vocab.char_vocab_size+vocab.tag_vocab_size, embedding_size, hidden_size, dropout_p=dropout_p)
        self.decoder = Decoder(vocab, embedding_size, hidden_size, use_ptr_gen, dropout_p=dropout_p)
        self.decoder.embedder.weight = self.encoder.embedder.weight # share weights
    ...
    def forward(self, lemma_indices, tag_indices, inflected_form_indices=None, a_ls_true=None, p_gens_true=None):
        ...
        h, mask, (h_n, c_n) = self.encoder([x + y for x, y in zip(lemma_indices, tag_indices)])

        s_0 = h_n  # (1, bsz, hidden_size)
        c_0 = c_n  # (1, bsz, hidden_size)

        input_indices_padded = pad_lists([x + y for x, y in zip(lemma_indices, tag_indices)], self.vocab.padding_idx, dtype=torch.long, device=device)
        ...
        if inflected_form_indices is not None:
            ...
            decoder_input = self.encoder.embedder(inflected_form_indices)  # (bsz, max_tgt_len, embedding_size)
        else:
            decoder_input = None

        p_ws, a_ls, p_gens = self.decoder(input_indices_padded, h, mask, (s_0, c_0), decoder_input, a_ls_true, p_gens_true)
